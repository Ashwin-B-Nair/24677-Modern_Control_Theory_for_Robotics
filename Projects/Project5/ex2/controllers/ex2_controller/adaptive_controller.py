...
class AdaptiveController(BaseController):
    ...
    def initializeGainMatrix(self):
        ...
        n_m = 4 # number of integral error terms
        Ap = np.zeros((n_p, n_p))
        Bp = np.zeros((n_p, n_m ))
        Cp = np.zeros((n_m, n_p))
        ...
        A = np.block([[Ap, np.zeros((n_p, n_m))], [Cp, np.zeros((n_m, n_m))]])
        B = np.block([[Bp], [np.zeros((n_m, n_m))]])
        C = np.block([Cp, np.zeros((n_m, n_m))])
        D = np.zeros((4,1))
        Bc = np.block([[np.zeros((n_p,n_m))],[-np.eye(n_m)]])
        A_d,B_d, _, _, _ = signal.cont2discrete((A, B, C, D), self.delT)
        _,Bc_d, _, _, _ = signal.cont2discrete((np.zeros_like(A), Bc, np.zeros_like(C), D), self.delT)
